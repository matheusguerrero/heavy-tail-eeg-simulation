...
patient_01 <- list(
  ...
  seizure_start = c(18, 19, 22),
  seizure_end   = c(62, 61, 58),
  ...
  tail_par = c(1.2, 0.8, 1.8),
  tail_band_weight = c(0.8, 0.5, 1.2, 1.0, 1.5),
...
p1_bands <- get_eeg_bands(p1)
bands <- c("delta", "theta", "alpha", "beta", "gamma")
for (i in 1:5) {
  print(plot_signal(p1, p1_bands[[i]], bands[i])) 
}

# Checking tail dependence via chi measure
plot_chi_win(p1, "chi", upp_quantile = .925)